 @if(isset($produto_detalhe->id))
            ...
            @else
                <form method="POST" action="{{route('produto_detalhe.store')}}"> 
                    @csrf
            @endif
                    <input type="text" name="produto_id" value="{{$produto_detalhe->produto_id ?? old('nome')}}" placeholder="Id do Produto" class="borda-preta">
                    {{$errors->has('produto_id') ? $errors->first('produto_id') : ""}}    
                    <input type="text" name="comprimento" value="{{$produto_detalhe->comprimento ?? old('descricao')}}" placeholder="Comprimento" class="borda-preta">
